...
const MobileHeader = ({
  ...
  // Clear accounts when dropdown opens
  useEffect(() => {
    if (showDropdown) {
      setAccounts([]);
    }
  ...
  return (
    <div className="relative flex items-center gap-3 px-5 md:hidden mb-0 pb-3" ref={dropdownRef}>
      {/* Avatar */}
      <button
      ...
      <div className="flex-1 relative">
        <form onSubmit={handleSearch} className="relative search-wrap">
          ...
          <input
            className="relative z-10 w-full bg-transparent py-2.5 pr-10 pl-10 text-sm placeholder-gray-400 dark:placeholder-gray-500 text-gray-900 dark:text-white border-none outline-none focus:ring-0"
        ...
        {/* Dropdown Results */}
        {showDropdown && (
          <Card forceRounded className="absolute top-full left-0 right-0 mt-2 max-h-[60vh] overflow-y-auto z-50 py-2 bg-white dark:bg-black">
            {!debouncedSearchText && (
