...
const SubmitOutcomeModal = ({ isOpen, onClose, applicationId, onSuccess }: SubmitOutcomeModalProps) => {
  ...
  const handleSubmit = async (data: z.infer<typeof SubmitOutcomeSchema>) => {
    ...
    try {
      ...
      const msg = error?.body?.message || error?.message || "Failed to submit work";
      toast.error(msg);
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        <div className="space-y-2">
          ...
          <div className="flex gap-4">
            <label className="flex items-center gap-2">
              ...
              />
              <span className="text-gray-700 text-sm dark:text-gray-300">Text</span>
            </label>
            <label className="flex items-center gap-2">
              <input
                type="radio"
                ...
                className="text-brand-500"
              />
              <span className="text-gray-700 text-sm dark:text-gray-300">File URL</span>
            </label>
          </div>
        </div>
        ...
        <TextArea
          label="Your Work"
          placeholder={form.watch("outcomeType") === "file" ? "Paste the URL to your work..." : "Describe or paste your completed work..."}
          rows={8}
